...
def compute_anisotropy(array_2d, type, plotter=True):
    ...
    if plotter:
        ...
        if type.lower() == 'local':
            ...
            ax_main.clabel(contour_lines_95, inline=True, fontsize=8, fmt='%.2f')
            ax_main.plot(data[:, 0], data[:, 1], 'k.', markersize=2, alpha=0.7)
    ...
    # Compute harmonic mean of the local anisotropy/anisotropies that may exist
...
def compare_boxplot(dictionary_list, y_label, custom_labels=None):
    ...
    if custom_labels is not None:
        ax.set_xticklabels(custom_labels, fontsize=22)
    else:
        ax.set_xticklabels(variable_names, fontsize=22)
    ax.tick_params(axis='y', labelsize=16)
    ax.set_ylabel(y_label, fontsize=22)
...
def compare_kde(dictionary, x_label, bandwidth='scott'):
    ...
    for idx, (label, values) in enumerate(dictionary.items()):
        sns.kdeplot(values, ax=ax, label=label, bw_method=bandwidth, color=colors[idx % len(colors)], lw=1.0)
